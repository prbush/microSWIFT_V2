...
gnss_error_code_t gnss_config(GNSS* self){
	...
	uint8_t config[159] =
	{0xB5,0x62,0x06,0x8A,0x97,0x00,0x01,0x01,0x00,0x00,
	 ...
	 0x04,0x00,0x93,0x10,0x00,0x01,0x00,0x21,0x30,0xFA,
	 ...
	 0x10,0x01,0x18,0x00,0x31,0x10,0x01,0x5C,0x25};
	...
	return_code = send_config(self, &(config[0]), sizeof(config));
		...
		size_t message_size)
{
	...
	uint16_t rx_length = FRAME_SYNC_RX_SIZE + 1;
	ULONG actual_flags;
	...
	while (frame_sync_attempts < MAX_FRAME_SYNC_ATTEMPTS) {
		register_watchdog_refresh();
		HAL_UARTEx_ReceiveToIdle(self->gnss_uart_handle, &(msg_buf[0]),
				FRAME_SYNC_RX_SIZE, &rx_length, ONE_SECOND);
